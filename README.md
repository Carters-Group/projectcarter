...
api/check-email.js              Vercel serverless function - proxies the enquiry form's email-deliverability check to abstractapi.com so ABSTRACT_API_KEY stays server-side (a Vercel env var), never in client-side JS
...
| Optional email-validation key (abstractapi.com, free tier) | Vercel project → Settings → Environment Variables → `ABSTRACT_API_KEY`, read server-side by `api/check-email.js` (skips the deliverability check while unset, everything else on the form still works) |
