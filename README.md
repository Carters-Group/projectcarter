...
dep-calculator.html             "Tax Depreciation (DEP)" - a desk estimate of an investment property's depreciation from four answers (type, new or established + year built, price, tax rate). Two deductions: Division 43 capital works (2.5% of original construction cost for 40 years; none for residential built on or before 15 Sep 1987) and Division 40 fixtures and fittings (a share of construction cost written off at 20% diminishing value; **nil for established residential unless bought before 9 May 2017**, with an on-page reason for every zero; established commercial assumes half of new value). Construction cost = the entered figure, else floor area x a build rate, else a share of the price, wound back to the build year at 3.5% pa. Shows a realistic range (-25%/+30%), 5 and 10 year totals, tax saved at the entered rate, a "same property if brand new" comparison and a QS-report payback note, plus a 10-year schedule. jsPDF one-pager, same reveal gate (`pc_dep_unlocked`) and Formspree ping as the others; `'dep'` added to the supabase CHECK and pc-report.js. Slug `/tax-depreciation-calculator`.
